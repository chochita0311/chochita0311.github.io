...
- Do not mix route-flash fixes from `prd-0006` into this track unless a split requires a tiny compatibility adjustment.

## Invariants

- Keep GitHub Pages compatible static delivery.
- Preserve current URL handling for root, `entry=archive`, category/collection filters, grid/list view, and note detail.
...
- Validation:
  ...
  - `?entry=archive` still bypasses landing.
